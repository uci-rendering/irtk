...
def str_to_tensor(s):
	...
	return torch.from_numpy(np.array(re.split(r',| ', s)).astype(np.float32))
...
class MitsubaParser(SceneParser):
	...
	# def read_emitter(self, e):
	# 	emitter_type = e.get('type')

	# 	if emitter_type == 'envmap':
	# 		if (str_n := e.find('emitter')) is not None
