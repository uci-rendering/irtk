...
def run(config):
    ...
    edge_length = config['init_edge_length'] / edge_length_decay
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for sensor_ids in sensor_batches:
            ...
            if iter % config['remesh_iter'] == 0:
                edge_length *= edge_length_decay
                V_data, F_data = remesh(V_data.detach().cpu().numpy(), F_data.cpu().numpy(), edge_length)
                V_data.require_grad = True
